  ...
  <div class="my-7 rounded bg-white p-7 filter drop-shadow-lg">
    <div id="header-project" class="flex flex-col md:flex-row">
      ...
      <div class="flex flex-row items-center flex-1 mt-2 md:mt-0">
        ...
        <p class="text-bold basis-1/2 text-nekasa-700">
          {{ progressTweened.toFixed(0) }}%
        </p>
      </div>
    </div>

    <p class="font-extralight text-nekasa-800 mt-5">
      {{ project.description }}
    </p>
        ...
        >
          <div class="flex">
            ...
            <font-awesome-icon
              v-else
              :icon="['far', 'circle']"
              class="text-nekasa-600 text-xl mt-1"
            />

            <div class="ml-3 flex flex-col w-full">
              <h2 class="text-lg font-semibold text-nekasa-600 mb-1">
                {{ task.title }}
              </h2>
